# modified to not run experiment
# modified model to pass m as parameter to calculate_cost and backward_prop
# modified predict to return array of values
...
def predict(X, parameters):
    a2, cache = forward_prop(X, parameters)
    yhat = a2
    yhat = list(np.squeeze(yhat))
    return [1 if n >= 0.5 else 0 for n in yhat]
